...
Suggested use of this scripts is by using the example shell scripts in the `etc` directory:
...
(Note that the scripts calls the DIP creation scripts as a module using python's `-m` flag, this is required due to the use of relative imports in the code)
